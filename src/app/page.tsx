...
/**
 * OneStepBehind – Notiz-App mit eigenen Kategorien + Stacks
 * - Firestore: categories, stacks, notes (alle per UID)
 * - Auth: anonym + Google-Link/Login
 * - Filter: dynamisch aus categories (inkl. "Alle")
 * - Neue Notiz: Kategorie-Chips + Stack-Auswahl (inkl. "(Kein Stack)"-Ersatz als neutraler leerer Badge), Ampel-Farbe
 *   → merkt sich pro Kategorie den zuletzt genutzten Stack
 * - Ansicht:
 *   • Filter = "Alle": klassische Liste
 *   • Filter = Kategorie: Stacks-Grid (Spalten)
 * - Edit / Delete, Glas-UI
 * - Kategorie löschen: X neben aktivem Kategorie-Chip (räumt Stacks & Note-Refs via Batch)
 */
    ...
    <div className="min-h-screen bg-white">
      <div className="p-8 max-w-5xl mx-auto font-sans">
        ...
        <div className="flex flex-col items-center mb-6">
          <Image src="/logo.png" alt="OneStepBehind Logo" width={88} height={88} priority />
        ...
        <div className="mb-6">
          <div className="mt-1 flex items-center gap-2">
            {isGoogle ? (
              ...
              <span className="text-xs px-2 py-1 rounded bg-gray-100 text-gray-700">Anonymer Benutzer</span>
            ) : (
              <span className="text-xs px-2 py-1 rounded bg-gray-100 text-gray-700">{userEmail}</span>
            )}
          </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {cols.map((col) => {
        ...
        const isRealStack = col.id !== noneKey;
        return (
  ...
  return (
    <div className={noteCardClass(note.color)}>
      ...
      <div className="flex items-center gap-3 mt-3 flex-wrap">
        ...
        {/* Stack-Badge: bei fehlendem Stack neutraler, leerer, klickbarer Badge (kein Text) */}
...
function GoogleGIcon() {
  return (
    <span className="inline-flex items-center gap-1 text-xs px-2 py-1 rounded bg-gray-100">
      <svg width="16" height="16" viewBox="0 0 533.5 544.3" aria-label="Google" role="img">
